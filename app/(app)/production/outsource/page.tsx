...
function OutsourceDetailPageContent() {
  ...
  const missingBatchesData = useQuery(api.outsourceBatches.checkMissingOutsourceBatches, {
    fiscalYear: currentFiscalYear
  });
              ...
              {sortedBatches.map((batch) => (
                <tr key={batch._id} className={`${batch.isUsed ? 'bg-green-100' : ''} ${batch.isOnHold ? 'bg-amber-50' : ''} hover:bg-gray-50`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-3">
                    <button
                      onClick={() => toggleOutsourceBatchHold({ batchId: batch._id, isOnHold: !batch.isOnHold })}
            ...
            <div className="mt-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900">
                  Outsource Reports ({sourceReports.length})
                </h3>
                <button
              ...
              <div className="max-h-96 overflow-y-auto">
                {sourceReports.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      No outsource reports have been uploaded yet.
                    </p>
                  </div>
                ) : (
...
export default function OutsourceDetailPage() {
  return (
    <ProtectedRoute route="/production/outsource">
      <OutsourceDetailPageContent />
    </ProtectedRoute>
  );
}
